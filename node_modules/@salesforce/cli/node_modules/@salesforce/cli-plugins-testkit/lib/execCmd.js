...
const determineExecutable = (cli = 'inherit') => {
    ...
    let bin = cli === 'inherit'
        ? kit_1.env.getString('TESTKIT_EXECUTABLE_PATH') ??
            (0, node_path_1.join)(process.cwd(), 'bin', process.platform === 'win32' ? 'dev.cmd' : 'dev.js')
        : cli;
    // Support plugins who still use bin/dev instead of bin/dev.js
    if (bin.endsWith('dev.js') && !fs.existsSync(bin))
        bin = bin.replace('.js', '');
...
async function execInteractiveCmd(command, answers, options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const child = (0, node_child_process_1.spawn)(bin, command.split(' '), opts);
