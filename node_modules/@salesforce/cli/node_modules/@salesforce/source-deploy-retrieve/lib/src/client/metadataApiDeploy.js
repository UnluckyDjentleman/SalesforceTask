...
const deployMessages_2 = require("./deployMessages");
const deployMessages_3 = require("./deployMessages");
;
...
const deleteNotFoundToFileResponses = (cs) => (messageMap) => Array.from(messageMap)
    ...
    .flatMap(([, messageArray]) => messageArray.filter(deployMessages_2.isComponentNotFoundWarningMessage))
    .flatMap((message) => {
...
const buildFileResponses = (response) => (0, kit_1.ensureArray)(response.details?.componentSuccesses)
    ...
    .map((c) => ({
    ...((0, deployMessages_2.getState)(c) === types_1.ComponentStatus.Failed
        ? {
            error: c.problem,
    ...
    state: (0, deployMessages_2.getState)(c),
...
const buildFileResponsesFromComponentSet = (cs) => (response) => {
    ...
    return (cs.getSourceComponents().toArray() ?? [])
        .flatMap((deployedComponent) => (0, deployMessages_1.createResponses)(deployedComponent, responseMessages.get((0, deployMessages_3.toKey)(deployedComponent)) ?? []).concat(deployedComponent.type.children
        ? deployedComponent.getChildren().flatMap((child) => {
            const childMessages = responseMessages.get((0, deployMessages_3.toKey)(child));
