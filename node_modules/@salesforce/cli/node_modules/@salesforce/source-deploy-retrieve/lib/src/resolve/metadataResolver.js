...
class MetadataResolver {
    ...
    getComponentsFromPathRecursive(dir, inclusiveFilter) {
        ...
        for (const fsPath of this.tree.readDirectory(dir).map((0, path_1.fnJoin)(dir))) {
...
const childSuffixMatches = (type, fsPath) => Object.values(type.children?.types ?? {})
    .map((childType) => `${childType.suffix}${constants_1.META_XML_SUFFIX}`)
    .some((s) => fsPath.endsWith(s));
/** the file suffix (in source or mdapi format) matches the type suffix we think it is */
const suffixMatches = (type, fsPath) => typeof type.suffix === 'string' && [type.suffix, `${type.suffix}${constants_1.META_XML_SUFFIX}`].some((s) => fsPath.endsWith(s));
