...
import { hasNestedRecords, } from '../../../dataSoqlQueryTypes.js';
...
export class ExportApi {
    ...
    ux;
    logger;
    objectTypeRegistry = {}; // registry for object type data plan descriptor
    refFromIdByType = new Map(); // refFromIdByType.get('account').get(someAccountId) => AccountRef1
    typeRefIndexes = new Map(); // registry for object type-specific ref counters
    config;
    constructor(org, ux) {
        this.org = org;
        this.ux = ux;
    ...
    async processQueryResults(recordList) {
        ...
        if (recordCount > 200 && !this.config.plan) {
            ...
            await Lifecycle.getInstance().emitWarning(messages.getMessage('dataExportRecordCountWarning', [recordCount, this.config.query]));
        }
