...
import { hasNonEmptyNestedRecords, hasNestedRecordsFilter, hasNestedRecords, } from './dataSoqlQueryTypes.js';
...
export const runExport = async (configInput) => {
    ...
    if (plan) {
        const planMap = reduceByType(recordsFromQuery
        ...
        const planFiles = [...planMap.entries()].map(([sobject, records]) => ({
            sobject,
            contents: { records },
        ...
        if (flatRecords.length > 200) {
            ...
            await Lifecycle.getInstance().emitWarning(messages.getMessage('dataExportRecordCountWarning', [flatRecords.length, query]));
        }
