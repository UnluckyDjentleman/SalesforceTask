...
export class DataSoqlQueryCommand extends SfCommand {
    ...
    async runBulkSoqlQuery(connection, query, timeout, allRows = false) {
        ...
        try {
            ...
            const res = (await bulk2.query(query, {
                pollTimeout: timeout.milliseconds ?? Duration.minutes(5).milliseconds,
                pollInterval: 5000,
                ...(allRows ? { scanAll: true } : {}),
            })) ?? [];
            return transformBulkResults((await res.toArray()), query);
        }
        catch (e) {
...
const prepareAsyncQueryResponse = (connection) => (cmd) => async ({ query, jobId }) => {
...
const buildEmptyQueryResult = ({ query, jobId }) => ({
    columns: [],
