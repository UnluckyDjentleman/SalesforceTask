...
class Compare extends snapshot_command_1.SnapshotCommand {
    ...
    async compareSnapshot(initialCommands, updatedCommands) {
        ...
        this.log(`${os_1.EOL}Command, flag, flag characters, or alias differences detected. If intended, please update the snapshot file and run again.`);
    ...
    get changed() {
        return this.commands.map(command => {
            return {
                ...
                chars: Object.values(command.flags).map(flag => flag.char).filter(char => typeof char === 'string').map(char => ({ name: char })),
            };
        });
    }
    async run() {
